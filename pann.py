...
class Network(object):
    ...
    def update_XY(self, X, Y):
        ...
        if not (X.shape[1] == self._shape[0] and
                Y.shape[1] == self._shape[-1]):
...
class AdaptiveAlpha(object):
    ...
    def optimize_alpha(self, alpha, show_plot=False):
        ...
        if sum_fit[min_ix] < sum_fit[alpha_ix]:
            if min_ix < alpha_ix:
                alpha_ix -= 1
            # don't increase except towards a global minimum
            elif min_ix > alpha_ix and fit_params[-1] > 0:
                alpha_ix += 1
...
def regen_string(s, shift_ix, key):
    if shift_ix is not None:
        caps = s[..., shift_ix]
    else:
        caps = [0] * s.shape[0]
    ...
    for char in s:
        ...
        if len(nonzero_tuple) > 0 and len(nonzero_tuple[0]) > 0:
            c_ix = nonzero_tuple[0][0]
        chars.append(key[c_ix])

    return ''.join(c.upper() if cap else c for c, cap in zip(chars, caps))

def markov_visualizer(network, n_cycles=500):
    ...
    print regen_string(start[0].reshape(-1, n_labels), shift_ix, key)[:-1]

    generated = []
    n = network.clone(start, fakeY)
    for i in range(n_cycles):
        s = start[0].reshape(-1, n_labels)
        generated.append(regen_string(s, shift_ix, key)[-1])
...
class TrainingData(object):
    def __init__(self, h5_data, batch_size, chunk_bytes=5 * 10 ** 9):
...
def build_parser():
    ...
    nn_parser.add_argument(
        '-I', '--training-input', type=str, required=True,
    ...
    nn_parser.add_argument(
        '-C', '--cv-input', type=str, metavar='h5_file',
